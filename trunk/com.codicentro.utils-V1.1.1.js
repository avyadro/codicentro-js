...
CJSUtil=new Object({	
	...
	suggestUsername:function(cfg){
		...
		splitName = cfg.fnm.toLowerCase().split(' ');
		suggest = [];		
		if(splitName.length==1){
			suggest.push(splitName[0].replace(cc,''));
		}else if(splitName.length==2){
		     suggest.push(splitName[0][0].replace(cc,'')+splitName[1].replace(cc,''));
		     suggest.push(splitName[0].substr(0,3).replace(cc,'')+splitName[1].replace(cc,''));
		     suggest.push(splitName[1].replace(cc,'')+splitName[0][0].replace(cc,''));
		}else if(splitName.length>=3){
		     suggest.push(splitName[0][0].replace(cc,'')+splitName[1].replace(cc,''));		     
		     suggest.push(splitName[0][0].replace(cc,'')+splitName[1][0].replace(cc,'')+splitName[2].replace(cc,''));
		     suggest.push(splitName[0][0].replace(cc,'')+splitName[1].replace(cc,'')+splitName[2][0].replace(cc,''));
		     suggest.push(splitName[0].substr(0,3).replace(cc,'')+splitName[1].replace(cc,''));
		     suggest.push(splitName[1].replace(cc,'')+splitName[0][0].replace(cc,''));		     
		}
	  return suggest;       
